/*
 * Copyright (C) 2016 Ronald Martin <hello@example.login.com>
 *
...
public class ViewPagerIndicator extends ViewGroup {
    ...
    @NonNull
    private final List<com.example.login.widget.IndicatorDotView> indicatorDots = new ArrayList<>();
    @NonNull
    private final List<com.example.login.widget.IndicatorDotPathView> dotPaths = new ArrayList<>();
    private com.example.login.widget.IndicatorDotView selectedDot;   // @NonNull, but initialized in init().
    @Px
    ...
    private void init(@NonNull Context context,
        ...
        final int defaultDotRadius = (int) (com.example.login.widget.IndicatorDotView.DEFAULT_DOT_RADIUS_DIP * scale + 0.5);
        ...
        unselectedDotColor = attributes.getColor(
                ...
                com.example.login.widget.IndicatorDotView.DEFAULT_UNSELECTED_DOT_COLOR
        );
        selectedDotColor = attributes.getColor(
                R.styleable.ViewPagerIndicator_selectedDotColor,
                com.example.login.widget.IndicatorDotView.DEFAULT_SELECTED_DOT_COLOR
        );
        ...
        selectedDot = new com.example.login.widget.IndicatorDotView(context);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (com.example.login.widget.IndicatorDotView indicatorDot : indicatorDots) {
            indicatorDot.measure(childWidthSpec, childHeightSpec);
        }
        for (com.example.login.widget.IndicatorDotPathView dotPath : dotPaths) {
    ...
    private void updateDotCount(int newDotCount) {
        ...
        if (dotCount < newDotCount) {
            ...
            while (dotCount++ != newDotCount) {
                final com.example.login.widget.IndicatorDotView newDot = new com.example.login.widget.IndicatorDotView(getContext());
            ...
            final List<com.example.login.widget.IndicatorDotView> removedDots =
                    new ArrayList<>(indicatorDots.subList(newDotCount, dotCount));
            for (com.example.login.widget.IndicatorDotView removedDot : removedDots) {
    ...
    private void updatePathCount(final int newPathCount) {
        ...
        if (pathCount < newPathCount) {
            ...
            while (pathCount++ != newPathCount) {
                final com.example.login.widget.IndicatorDotPathView newPath = new com.example.login.widget.IndicatorDotPathView(
            ...
            final List<com.example.login.widget.IndicatorDotPathView> pathsToRemove =
                    new ArrayList<>(dotPaths.subList(newPathCount, pathCount));
            for (com.example.login.widget.IndicatorDotPathView dotPath : pathsToRemove) {
    ...
    private void updateIndicatorPositions(int currentPage, float positionOffset, boolean forceUpdate) {
        ...
        for (int i = 0,
            ...
            final com.example.login.widget.IndicatorDotView dotView = indicatorDots.get(i);
            dotView.layout(left, top, right, bottom);

            if (i < pathCount)  {
                final com.example.login.widget.IndicatorDotPathView dotPath = dotPaths.get(i);
    ...
    private Animator pageChangeAnimator(final int lastPageIndex, final int newPageIndex) {
        final com.example.login.widget.IndicatorDotPathView dotPath = getDotPathForPageChange(lastPageIndex, newPageIndex);
        final com.example.login.widget.IndicatorDotView lastDot = getDotForPage(lastPageIndex);
    ...
    private Animator selectedDotSlideAnimator(int newPageIndex,
        ...
        final com.example.login.widget.IndicatorDotView newPageDot = getDotForPage(newPageIndex);
        if (newPageDot != null) {
    ...
    @Nullable
    private com.example.login.widget.IndicatorDotView getDotForPage(int pageIndex) {
        if (pageIndex > indicatorDots.size() - 1 || pageIndex < 0) return null;
        return indicatorDots.get(pageIndex);
    }

    @Nullable
    private com.example.login.widget.IndicatorDotPathView getDotPathForPageChange(int oldPageIndex, int newPageIndex) {
    ...
    @com.example.login.widget.IndicatorDotPathView.PathDirection
    private int getPathDirectionForPageChange(int oldPageIndex, int newPageIndex) {
        return oldPageIndex < newPageIndex ? com.example.login.widget.IndicatorDotPathView.PATH_DIRECTION_RIGHT :
                com.example.login.widget.IndicatorDotPathView.PATH_DIRECTION_LEFT;
    }
